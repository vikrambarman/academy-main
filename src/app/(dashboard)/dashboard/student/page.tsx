...
interface Course {
    ...
    authority?: string; verification?: string;
}

interface FranchiseData {
    _id: string; name: string; code: string;
    registeredBodies: string[];
    portalUrl?: string; portalLoginRequired: boolean; isOwn: boolean;
}

interface CertTypeData {
    _id: string; name: string; code: string;
    issuingBody: string; verificationMethod: string;
    verificationUrl?: string; benefits: string[];
}
...
interface Enrollment {
    ...
    certificateStatus: string; payments: Payment[];
    course: Course;
...
function CertBadge({ status }: { status: string }) {
    ...
    if (s === "certificate generated") return (
        <span style={{ display: "inline-flex", alignItems: "center", gap: 5, background: "rgba(34,197,94,0.12)", color: "#4ADE80", fontSize: 12, fontWeight: 600, padding: "5px 12px", borderRadius: 100 }}>
            <CheckCircle2 size={12} /> Issued
        </span>
    );
    if (s === "passed") return (
        <span style={{ display: "inline-flex", alignItems: "center", gap: 5, background: "rgba(34,197,94,0.08)", color: "#4ADE80", fontSize: 12, fontWeight: 600, padding: "5px 12px", borderRadius: 100 }}>
            <CheckCircle2 size={12} /> Passed
        </span>
    );
    if (s === "applied" || s === "exam given") return (
        <span style={{ display: "inline-flex", alignItems: "center", gap: 5, background: "rgba(26,86,219,0.12)", color: "var(--sp-accent2)", fontSize: 12, fontWeight: 600, padding: "5px 12px", borderRadius: 100 }}>
            <Clock size={12} /> {status}
        </span>
    );
    return (
        <span style={{ display: "inline-flex", alignItems: "center", gap: 5, background: "rgba(100,116,139,0.12)", color: "#94A3B8", fontSize: 12, fontWeight: 600, padding: "5px 12px", borderRadius: 100 }}>
            <Award size={12} /> {status}
        </span>
    );
}

/* ── Franchise info strip shown inside enrollment card ── */
function FranchiseStrip({ franchise, certType, externalStudentId }: {
    franchise: FranchiseData;
    certType?: CertTypeData | null;
    externalStudentId?: string | null;
}) {
    const accentColor = franchise.isOwn ? "#F59E0B" : "var(--sp-accent)";
    const accentBg    = franchise.isOwn ? "rgba(245,158,11,0.08)" : "rgba(26,86,219,0.08)";

    return (
        <div style={{
            background:   accentBg,
            border:       `1px solid ${franchise.isOwn ? "rgba(245,158,11,0.2)" : "rgba(26,86,219,0.2)"}`,
            borderRadius: 10,
            padding:      "12px 14px",
            marginBottom: 18,
        }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
                ...
                <span style={{ fontSize: 12, fontWeight: 700, color: "var(--sp-text)" }}>
                    {franchise.name}
                </span>
                {franchise.isOwn && (
            ...
            {/* Registered bodies */}
            <div style={{ display: "flex", flexWrap: "wrap", gap: 4, marginBottom: certType ? 8 : 0 }}>
            ...
            {certType && (
                <div style={{
                    ...
                    background: "var(--sp-surface)", borderRadius: 7,
                    border: "1px solid var(--sp-border)",
                }}>
                    <Award size={12} style={{ color: accentColor, flexShrink: 0, marginTop: 1 }} />
                    <div>
                        <div style={{ fontSize: 11, fontWeight: 700, color: "var(--sp-text)", marginBottom: 1 }}>
                            {certType.name}
                        </div>
                        <div style={{ fontSize: 10, color: "var(--sp-muted)" }}>
                            {certType.issuingBody}
                            {certType.verificationMethod ? ` · ${certType.verificationMethod}` : ""}
                        </div>
                    </div>
                </div>
            )}
            ...
            {/* External student ID */}
            {externalStudentId && (
                <div style={{ marginTop: 7, fontSize: 11, color: "var(--sp-subtext)" }}>
                    <span style={{ color: "var(--sp-muted)", fontWeight: 600 }}>{franchise.code} ID: </span>
                    <span style={{ fontFamily: "monospace", color: "var(--sp-text)" }}>{externalStudentId}</span>
                </div>
            )}
        </div>
    );
    ...
    useEffect(() => {
        fetchWithAuth("/api/student/profile")
            .then(async res => {
                ...
                // Shape A (new): { student: {...}, enrollments: [...] }
                // Shape B (old): { name, studentId, enrollments: [...] }
                if (json?.student) {
                    setData(json);
    ...
    if (accountDisabled) return (
        <div style={{ maxWidth: 480, margin: "48px auto", fontFamily: "'Plus Jakarta Sans',sans-serif" }}>
            <div style={{ background: "rgba(239,68,68,0.08)", border: "1px solid rgba(239,68,68,0.2)", borderRadius: 16, padding: "32px 28px", textAlign: "center" }}>
                ...
                <div style={{ fontSize: 13, fontWeight: 300, color: "var(--sp-subtext)", lineHeight: 1.7 }}>
                ...
                {enrollments.length === 0 ? (
                    ...
                    <>
                        ...
                        {enrollments.map(e => {
                            ...
                            // Authority display: franchise name if available, else course.authority
                            const authorityDisplay = e.franchise
                                ? e.franchise.name
                                : e.course?.authority;

                            // Verify URL: certType first, then course verification
                            const verifyUrl = e.certType?.verificationUrl
                                ...
                                <div key={e._id} className={`sd-enroll-card ${expanded ? "expanded" : ""}`}>
                                    ...
                                    <div className="sd-enroll-header" onClick={() => toggleCard(e._id)}>
                                        <div style={{ flex: 1, minWidth: 0 }}>
                                            ...
                                            <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 3, flexWrap: "wrap" }}>
                                                {/* Franchise badge (new) or authority text (legacy) */}
                                                {e.franchise ? (
                                                    <span style={{
                                                    ...
                                                    <span className="sd-course-auth">{authorityDisplay}</span>
                                                ) : null}

                                                {/* Cert type short name */}
                                                {e.certType && (
                                                    <span style={{ fontSize: 10, color: "var(--sp-muted)", fontWeight: 500 }}>
                                                        {e.certType.code}
                                                    </span>
                                                )}
                                            </div>
                                    ...
                                    {expanded && (
                                        <div className="sd-enroll-body">
                                            ...
                                            {/* Franchise strip — only for new enrollments with franchise */}
                                            {e.franchise && (
                                                <FranchiseStrip
                                            ...
                                            {/* Certificate status + verify link */}
                                            <div className="sd-cert-row">
                                                ...
                                                {verifyUrl && (
                                                    <a href={verifyUrl} target="_blank" rel="noopener noreferrer" className="sd-verify-link">
...
const sdStyles = `
    ...
    .sd-welcome::before { content: ''; position: absolute; right: -40px; top: -40px; width: 200px; height: 200px; background: rgba(255,255,255,0.06); border-radius: 50%; }
    .sd-welcome::after  { content: ''; position: absolute; right: 60px; bottom: -60px; width: 160px; height: 160px; background: rgba(255,255,255,0.04); border-radius: 50%; }
    ...
    .sd-stat-card { background: var(--sp-surface); border: 1px solid var(--sp-border); border-radius: 14px; padding: 16px 18px; position: relative; overflow: hidden; }
    ...
    .sd-section-title { font-size: 13px; font-weight: 700; color: var(--sp-muted); letter-spacing: 0.08em; text-transform: uppercase; display: flex; align-items: center; gap: 8px; margin-bottom: 14px; }
    .sd-section-title::after { content: ''; flex: 1; height: 1px; background: var(--sp-border); }

    .sd-enroll-card { background: var(--sp-surface); border: 1px solid var(--sp-border); border-radius: 16px; overflow: hidden; margin-bottom: 14px; transition: box-shadow 0.2s; }
    .sd-enroll-card:hover { box-shadow: 0 4px 24px rgba(26,86,219,0.12); }
    .sd-enroll-header { padding: 18px 22px; display: flex; align-items: center; justify-content: space-between; gap: 12px; cursor: pointer; border-bottom: 1px solid transparent; transition: border-color 0.18s, background 0.18s; }
    .sd-enroll-card.expanded .sd-enroll-header { border-bottom-color: var(--sp-border); background: var(--sp-hover); }
    .sd-course-name { font-size: 15px; font-weight: 700; color: var(--sp-text); margin-bottom: 2px; }
    ...
    .sd-chevron-btn { width: 28px; height: 28px; border-radius: 8px; border: 1px solid var(--sp-border); background: transparent; cursor: pointer; display: flex; align-items: center; justify-content: center; color: var(--sp-muted); transition: background 0.15s; }
    ...
    .sd-mini-kpi-item { background: var(--sp-hover); border: 1px solid var(--sp-border); border-radius: 10px; padding: 12px 14px; }
    ...
    .sd-verify-link { display: inline-flex; align-items: center; gap: 6px; font-size: 12px; font-weight: 600; color: var(--sp-accent2); text-decoration: none; padding: 7px 14px; border: 1px solid var(--sp-border2); border-radius: 8px; transition: background 0.15s; }
    ...
    .sd-payment-row { display: flex; align-items: center; justify-content: space-between; padding: 11px 14px; gap: 12px; border-bottom: 1px solid var(--sp-border); transition: background 0.14s; }
    ...
    .sd-empty { background: var(--sp-surface); border: 1px solid var(--sp-border); border-radius: 16px; padding: 48px 24px; text-align: center; }
