...
import {
    ...
    ChevronLeft, ChevronRight, QrCode, Edit3, LogIn, LogOut
} from "lucide-react";
        ...
        <>
            ...
            <div className="sap-root">
                ...
                <div>
                    ...
                    <p className="sap-sub">Course-wise attendance aur detailed stats dekho</p>
                </div>
                    ...
                    <div className="sap-ring-card" style={{ borderColor: pctBorder, background: pctBg }}>
                        <div className="sap-ring-wrap">
                            <svg width="100" height="100" viewBox="0 0 100 100">
                                ...
                                <circle
                                    cx="50" cy="50" r="36" fill="none" stroke={pctColor} strokeWidth="9"
                ...
                {months.length === 0 ? (
                    <div className="sap-no-records">
                        <CalendarDays size={22} style={{ opacity: .3 }} /> Abhi tak koi record nahi
                    </div>
                ) : (
                    <div className="sap-records-card">
                        {/* Month Nav */}
                        <div className="sap-month-nav">
                            <button className="sap-nav-btn" disabled={month === 0} onClick={() => setMonth(m => m - 1)}>
                                <ChevronLeft size={15} />
                            </button>
                            <div className="sap-month-label">
                                <CalendarDays size={13} color="var(--sp-accent)" />
                                {curMonth?.label}
                            </div>
                            <button className="sap-nav-btn" disabled={month === months.length - 1} onClick={() => setMonth(m => m + 1)}>
                        ...
                        <div className="sap-thead">
                            <span>Date</span>
                            <span>Status</span>
                            <span className="sap-col-time">In → Out</span>
                            <span className="sap-col-duration">Duration</span>
                            <span className="sap-col-marked">Marked Via</span>
                        </div>
                        ...
                        {curMonth?.records.length === 0 ? (
                            ...
                            return (
                                <div key={i} className="sap-row">
                                    {/* Date */}
                                    <div className="sap-col-date">{fmtDate(rec.date)}</div>
                                        ...
                                        {rec.inTime || rec.outTime ? (
                                            <div className="sap-time-row">
                                                <span className="sap-time-in">
                                                    <LogIn size={11} style={{ opacity: 0.5 }} />
                                                    {rec.inTime || "—"}
                                                </span>
                                                <span style={{ opacity: 0.3 }}>→</span>
                                                <span className="sap-time-out">
                                                    <LogOut size={11} style={{ opacity: 0.5 }} />
                                                    {rec.outTime || "—"}
                                                </span>
                                            </div>
                                        ) : (
                                            <span style={{ color: "var(--sp-muted)", fontSize: 11 }}>—</span>
                                        )}
                                    </div>
                                    ...
                                    <div className="sap-col-duration">
                                        {duration !== "—" ? (
                                            <span className="sap-duration-badge">{duration}</span>
                                        ) : (
                                            <span style={{ color: "var(--sp-muted)", fontSize: 11 }}>—</span>
                                        )}
                                    </div>
                                    ...
                                    <div className="sap-col-marked">
                                        {rec.markedVia === "qr" ? (
                                            <span className="sap-marked-badge qr">
                                                <QrCode size={10} /> QR
                                            </span>
                                        ) : rec.markedVia === "manual" ? (
                                            <span className="sap-marked-badge manual">
...
const attStyles = `
  ...
  .sap-thead { 
    display: grid; 
    grid-template-columns: 1.3fr 100px 130px 85px 90px; 
    gap: 8px; 
    padding: 9px 18px; 
  ...
  /* Time columns */
  .sap-time-row { display: flex; align-items: center; gap: 5px; font-size: 11px; color: var(--sp-text); }
  .sap-time-in, .sap-time-out { display: flex; align-items: center; gap: 3px; }
  ...
  @media (max-width: 768px) {
    ...
    .sap-thead, .sap-row { 
      grid-template-columns: 1.2fr 90px 110px; 
    }
    .sap-col-duration, .sap-col-marked { display: none; }
    .sap-time-row { flex-direction: column; align-items: flex-start; gap: 2px; }
  }

  @media (max-width: 500px) {
    .sap-thead, .sap-row { grid-template-columns: 1fr 85px; }
    .sap-col-time { display: none; }
  }
`;