...
import {
    ...
    GraduationCap, Info, Hash, Calendar, Link2
} from "lucide-react";
...
// ── Certificate record jo admin ne add kiya ──────────────────────────────────
...
interface Enrollment {
    ...
    certificate:       CertRecord | null;  // ← ab yahan attached hai
}
...
function getAuthorityInfo(authority?: string): AuthorityInfo {
    ...
    if (a.includes("nsdc") || a.includes("skill india") || a.includes("pmkvy")) return {
        ...
        helpText: "Certificate issued under NSDC / Skill India framework.",
...
function CertStatusBadge({ status }: { status: string }) {
    ...
    if (s === "issued" || s === "certificate generated") return (
...
function CertDetailsCard({ cert, auth }: { cert: CertRecord; auth: AuthorityInfo }) {
    ...
    return (
        <div style={{
            ...
            <div style={{
                ...
                marginBottom: 14,
                paddingBottom:14,
                borderBottom: `1px solid ${auth.border}`,
            }}>
                <div style={{
                    width:          36, height: 36,
                    borderRadius:   9,
                ...
                <div>
                    <div style={{ fontSize:9, fontWeight:700, letterSpacing:"0.1em", textTransform:"uppercase", color:auth.color, marginBottom:2 }}>
                        Certificate Number
                    </div>
                    <div style={{
            ...
            {/* Details grid */}
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginBottom: cert.verifyUrl || cert.remarks ? 12 : 0 }}>
                <div>
                    <div style={{ fontSize:9, fontWeight:700, letterSpacing:"0.1em", textTransform:"uppercase", color:"var(--sp-muted)", marginBottom:3 }}>
                        Issue Date
                    </div>
                    <div style={{ fontSize:12, fontWeight:600, color:"var(--sp-text)", display:"flex", alignItems:"center", gap:5 }}>
                        <Calendar size={11} style={{ color:auth.color }}/> {fmtDate(cert.issueDate)}
                    </div>
                </div>
                {cert.expiryDate && (
                    <div>
                        ...
                        <div style={{ fontSize:12, fontWeight:600, color:"var(--sp-text)", display:"flex", alignItems:"center", gap:5 }}>
                            <Calendar size={11} style={{ color:auth.color }}/> {fmtDate(cert.expiryDate)}
                        </div>
                    </div>
                )}
                <div>
                    ...
                    <div style={{ fontSize:12, fontWeight:600, color:"var(--sp-text)" }}>
                        {cert.authority}
                    </div>
                </div>
                <div>
                    ...
                    <div style={{ fontSize:12, fontWeight:700, color: cert.status === "issued" ? "var(--sp-success)" : cert.status === "revoked" ? "var(--sp-danger)" : "var(--sp-warn)", textTransform:"capitalize" }}>
                        {cert.status}
                    </div>
                </div>
            </div>
            ...
            {cert.verifyUrl && (
                <a
                    ...
                    style={{
                        ...
                        color:          auth.color,
                        textDecoration: "none",
                        marginBottom:   cert.remarks ? 10 : 0,
                        padding:        "5px 10px",
                ...
                >
                    <Link2 size={11}/> Direct Verify Link <ExternalLink size={10}/>
                </a>
            )}
...
export default function StudentCertificates() {
    ...
    if (loading) return (
        <div style={{ padding:"48px 0", display:"flex", flexDirection:"column", alignItems:"center", gap:14 }}>
            ...
            <span style={{ fontSize:13, color:"var(--sp-muted)", fontFamily:"'Plus Jakarta Sans',sans-serif" }}>
    ...
    const issuedCount  = enrollments.filter(e => e.certificate?.status === "issued").length;
                ...
                {enrollments.length === 0 ? (
                    <div className="sc-empty">
                        ...
                        <div className="sc-empty-sub">
                            Your certificates will appear here once you are enrolled in a course.
                        </div>
                    </div>
                ) : enrollments.map(e => {
                    const auth   = getAuthorityInfo(e.certificate?.authority || e.course?.authority);
                        ...
                        <div key={e._id} className="sc-cert-card">
                            ...
                            <div className="sc-cert-band" style={{ background: auth.bg }}>
                                ...
                                <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                                    ...
                                    <CertStatusBadge status={e.certificate?.status || e.certificateStatus}/>
                                </div>
                            </div>
                            ...
                            <div className="sc-cert-body">
                                ...
                                {/* ── Certificate details (admin ne add kiye) ── */}
                                {e.certificate && issued && (
                                    <CertDetailsCard cert={e.certificate} auth={auth}/>
                                )}
                                        ...
                                        <div className="sc-steps">
                                            {auth.steps.map((step, i) => (
                                                <div key={i} className="sc-step">
                                                    <div className="sc-step-num" style={{ background:auth.bg, color:auth.color, border:`1px solid ${auth.border}` }}>
                                                        {i + 1}
                                                    </div>
                                                    {step}
                                                </div>
                                ...
                                <div className="sc-actions">
                                    {/* Admin ka specific verifyUrl pehle use karo */}
                                    {issued && e.certificate?.verifyUrl && (
                                        <a href={e.certificate.verifyUrl} target="_blank" rel="noopener noreferrer" className="sc-btn-primary">
                                            <Search size={13}/> Verify Certificate <ExternalLink size={11}/>
                                        </a>
                                    )}
                                    {/* Course ka verification URL fallback */}
                                    {issued && !e.certificate?.verifyUrl && e.course?.verification && (
                                    ...
                                    {issued && !e.certificate?.verifyUrl && !e.course?.verification && !e.course?.externalPortalUrl && auth.portalUrl && (
