...
export default function TeacherAttendancePage() {
        ...
        try {
            ...
            const rows: Student[] = enrs.map((e: any) => {
                ...
                return {
                    ...
                    stats:   att?.stats ?? { percentage:0, present:0, total:0 },
                };
            });
            setStudents(rows);
    ...
    const updateStatus = (id: string, status: Status) => setStudents(s => s.map(st => st.enrollmentId === id ? { ...st, status } : st));
    const updateRemark = (id: string, remark: string) => setStudents(s => s.map(st => st.enrollmentId === id ? { ...st, remark } : st));
    const handleMarkAll = (status: Status) => { setMarkAll(status); setStudents(s => s.map(st => ({ ...st, status }))); };
    ...
    const handleSave = async () => {
        ...
        setSaving(true);
        try {
            const records = students.map(s => ({ enrollmentId:s.enrollmentId, studentId:s.enrollmentId, courseId, status:s.status, remark:s.remark }));
    ...
    return (
        <>
            <style>{`
                .ta-root { display:flex; flex-direction:column; gap:18px; max-width:960px; margin:0 auto; padding-bottom:48px; font-family:'Plus Jakarta Sans',sans-serif; color:var(--tp-text); }
                ...
                .ta-input   { background:var(--tp-bg); border:1px solid var(--tp-border); border-radius:9px; padding:9px 12px; font-family:'Plus Jakarta Sans',sans-serif; font-size:13px; color:var(--tp-text); outline:none; transition:border-color .13s; }
                .ta-input:focus { border-color:var(--tp-accent2); }

                .ta-mark-all { display:flex; align-items:center; gap:8px; flex-wrap:wrap; }
                .ta-mark-all-label { font-size:11px; font-weight:700; color:var(--tp-muted); margin-right:4px; }
                .ta-mark-btn { display:flex; align-items:center; gap:5px; font-family:'Plus Jakarta Sans',sans-serif; font-size:11px; font-weight:700; padding:5px 12px; border-radius:100px; border:1px solid var(--tp-border); background:transparent; color:var(--tp-muted); cursor:pointer; transition:all .13s; }
                .ta-mark-btn:hover { border-color:var(--tp-border2); color:var(--tp-subtext); }

                .ta-summary      { display:flex; align-items:center; gap:16px; flex-wrap:wrap; background:var(--tp-surface); border:1px solid var(--tp-border); border-radius:11px; padding:12px 18px; }
                ...
                .ta-summary-key  { font-size:10px; font-weight:600; text-transform:uppercase; letter-spacing:.06em; color:var(--tp-muted); }
                .ta-summary-total { margin-left:auto; display:flex; align-items:center; gap:6px; font-size:12px; font-weight:700; color:var(--tp-muted); }

                .ta-list { display:flex; flex-direction:column; gap:8px; }
                .ta-row  { background:var(--tp-surface); border:1px solid var(--tp-border); border-left:3px solid var(--tp-border); border-radius:12px; padding:13px 16px; display:flex; align-items:flex-start; gap:14px; flex-wrap:wrap; transition:border-left-color .15s; }
                ...
                .ta-sno    { font-size:11px; font-weight:700; color:var(--tp-border2); width:20px; text-align:right; flex-shrink:0; }
                .ta-avatar { width:32px; height:32px; border-radius:50%; background:var(--tp-accent-glow); color:var(--tp-accent2); display:flex; align-items:center; justify-content:center; font-size:13px; font-weight:800; flex-shrink:0; border:1px solid var(--tp-accent-b); }
                .ta-info   { flex:1; }
                .ta-name   { font-size:13px; font-weight:700; color:var(--tp-text); }
                ...
                .ta-status-btn  { display:flex; align-items:center; gap:5px; font-family:'Plus Jakarta Sans',sans-serif; font-size:11px; font-weight:700; padding:5px 11px; border-radius:100px; border:1px solid var(--tp-border); background:transparent; color:var(--tp-muted); cursor:pointer; transition:all .12s; }
                .ta-status-btn:hover { border-color:var(--tp-border2); color:var(--tp-subtext); }

                .ta-remark { background:var(--tp-bg); border:1px solid var(--tp-border); border-radius:8px; padding:6px 10px; font-family:'Plus Jakarta Sans',sans-serif; font-size:11px; color:var(--tp-subtext); outline:none; width:100%; }
                .ta-remark:focus { border-color:var(--tp-accent2); }

                .ta-bottom-bar  { display:flex; align-items:center; justify-content:space-between; padding:13px 18px; background:var(--tp-surface); border:1px solid var(--tp-border); border-radius:12px; }
            ...
            <div className="ta-root">
                ...
                <div className="ta-header">
                    <div><h1 className="ta-title">Attendance</h1><p className="ta-sub">Daily student attendance mark karo</p></div>
                    {students.length > 0 && (
                ...
                {!courseId ? (
                    ...
                    <div className="ta-list">
                        {students.map((st, i) => {
                            ...
                            return (
                                <div key={st.enrollmentId} className="ta-row" style={cfg ? { borderLeftColor:cfg.color } : {}}>
                                    <div className="ta-row-left">
                                        ...
                                        <div className="ta-pct" style={{ color: st.stats.percentage>=75 ? "#22c55e" : st.stats.percentage>=50 ? "#f59e0b" : "#f87171" }}>
                                            {st.stats.percentage}%
                                        </div>
                                    </div>
                ...
                {students.length > 0 && (
                    <div className="ta-bottom-bar">
                        <span className="ta-bottom-info">
                            {summary.unmarked > 0
                                ? <><AlertCircle size={12} color="var(--tp-warn)"/> {summary.unmarked} students unmarked</>
                                : <><CheckCircle size={12} color="var(--tp-success)"/> Sab mark ho gaye</>
                            }
                        </span>
                        <button className="ta-save-btn" onClick={handleSave} disabled={saving}>
                            <Save size={13}/> {saving ? "Saving..." : "Save Attendance"}
