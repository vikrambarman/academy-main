...
import { TrendingUp, Users, BookOpen, IndianRupee, RefreshCw, Shield } from "lucide-react";
...
interface Analytics {
    ...
    // New: franchise breakdown (from analytics API)
...
const CpTooltip = ({ active, payload, label }: any) => {
    ...
    return (
        <div style={{
            ...
            {label && <div style={{ color: "var(--cp-muted)", marginBottom: 4, fontWeight: 700 }}>{label}</div>}
            {payload.map((p: any, i: number) => (
                <div key={i} style={{ color: p.color || "var(--cp-text)", display: "flex", gap: 8, alignItems: "center" }}>
            ...
            <div className="ar-root">
                ...
                <div className="ar-kpi-row">
                    <div className="ar-kpi amber">
                        <div className="ar-kpi-icon"><Users size={16} style={{ color: "var(--cp-warning)" }} /></div>
                        <div>
                    ...
                    <div className="ar-kpi green">
                        <div className="ar-kpi-icon"><IndianRupee size={16} style={{ color: "var(--cp-success)" }} /></div>
                        <div>
                    ...
                    <div className="ar-kpi red">
                        <div className="ar-kpi-icon"><TrendingUp size={16} style={{ color: "var(--cp-danger)" }} /></div>
                        <div>
                ...
                {/* Franchise Breakdown — only show if data available */}
                {data?.franchiseStats && data.franchiseStats.length > 0 && (
                    <div className="ar-chart-card" style={{ maxWidth: "100%" }}>
                        <div className="ar-chart-head">
                            <span className="ar-chart-title">
                        ...
                        <div className="ar-chart-body">
                            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(220px, 1fr))", gap: 10 }}>
                                {data.franchiseStats.map((f, i) => {
                                    const pct = f.totalFees > 0 ? Math.round((f.collectedFees / f.totalFees) * 100) : 0;
                                    const color = f.isOwn ? "#F59E0B" : COLORS[i % COLORS.length];
                                    return (
                                        <div key={f.franchiseId} style={{
                                            ...
                                            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 10 }}>
                                                <div>
                                                    <div style={{ fontSize: 9, fontWeight: 700, textTransform: "uppercase", letterSpacing: ".08em", color: "var(--cp-muted)", marginBottom: 3 }}>Students</div>
                                                    <div style={{ fontSize: 18, fontFamily: "'DM Serif Display',serif", color: "var(--cp-text)" }}>{f.studentCount}</div>
                                                </div>
                                                <div>
                                                    <div style={{ fontSize: 9, fontWeight: 700, textTransform: "uppercase", letterSpacing: ".08em", color: "var(--cp-muted)", marginBottom: 3 }}>Collected</div>
                                                    <div style={{ fontSize: 13, fontWeight: 700, color: "var(--cp-success)" }}>{fmt(f.collectedFees)}</div>
                                                </div>
                                            </div>
                                            {/* Fee progress */}
                                            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                                                <div style={{ flex: 1, height: 4, background: "var(--cp-border)", borderRadius: 100, overflow: "hidden" }}>
                                                    <div style={{ height: "100%", width: `${pct}%`, background: color, borderRadius: 100, transition: "width .5s" }} />
                                                </div>
                                                <span style={{ fontSize: 10, fontWeight: 700, color: "var(--cp-muted)", flexShrink: 0 }}>{pct}%</span>
                                            </div>
                                        </div>
                                    );
                                })}
                        ...
                        <div className="ar-chart-body">
                            <ResponsiveContainer width="100%" height={220}>
                                <LineChart data={data?.revenueByMonth || []} margin={{ top: 5, right: 10, left: 0, bottom: 0 }}>
                                    <XAxis dataKey="month" tick={{ fill: "var(--cp-muted)", fontSize: 10 }} axisLine={false} tickLine={false} />
                                    <YAxis tick={{ fill: "var(--cp-muted)", fontSize: 10 }} axisLine={false} tickLine={false} tickFormatter={v => `₹${(v / 1000).toFixed(0)}k`} />
                                    <Tooltip content={<CpTooltip />} />
                                    <Legend wrapperStyle={{ fontSize: 11, color: "var(--cp-muted)" }} />
                                    <Line type="monotone" dataKey="revenue" name="Revenue" stroke="var(--cp-success)" strokeWidth={2} dot={false} />
                                    <Line type="monotone" dataKey="pending" name="Pending" stroke="var(--cp-danger)" strokeWidth={2} dot={false} strokeDasharray="4 2" />
                                </LineChart>
                            </ResponsiveContainer>
                        </div>
                    </div>
                        ...
                        <div className="ar-chart-body">
                            <ResponsiveContainer width="100%" height={220}>
                                <BarChart data={data?.enrollmentsByMonth || []} margin={{ top: 5, right: 10, left: 0, bottom: 0 }}>
                                    <XAxis dataKey="month" tick={{ fill: "var(--cp-muted)", fontSize: 10 }} axisLine={false} tickLine={false} />
                                    <YAxis tick={{ fill: "var(--cp-muted)", fontSize: 10 }} axisLine={false} tickLine={false} allowDecimals={false} />
                                    <Tooltip content={<CpTooltip />} />
                                    <Bar dataKey="count" name="Enrollments" fill="var(--cp-warning)" radius={[4, 4, 0, 0]} />
                                </BarChart>
                            </ResponsiveContainer>
                        </div>
                    </div>
                            ...
                            <div style={{ flex: 1, minWidth: 120, display: "flex", flexDirection: "column", gap: 6 }}>
                                {data?.courseWiseStudents?.map((c, i) => (
                                    <div key={i} style={{ display: "flex", alignItems: "center", gap: 7 }}>
                                        ...
                                        <span style={{ fontSize: 11, color: "var(--cp-subtext)", flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                                            {c.course}
                                        </span>
                                        <span style={{ fontSize: 11, fontWeight: 700, color: "var(--cp-text)" }}>{c.count}</span>
                                    </div>
                                ))}
                            </div>
                        </div>
                    ...
                    <div className="ar-chart-card wide">
                        ...
                        <div className="ar-chart-body">
                            {!data?.attendanceSummary?.length ? (
                                <div style={{ color: "var(--cp-muted)", fontSize: 12, padding: "20px 0", textAlign: "center" }}>
                                    No attendance data
                                </div>
                            ) : (
                                    ...
                                    {data.attendanceSummary.slice(0, 10).map((s, i) => {
                                        ...
                                        return (
                                            <div key={i} style={{ display: "flex", alignItems: "center", gap: 10 }}>
                                                <span style={{ fontSize: 11, color: "var(--cp-muted)", width: 130, flexShrink: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                                                    {s.name}
                                                </span>
                                                <div style={{ flex: 1, height: 5, background: "var(--cp-surface2)", borderRadius: 100, overflow: "hidden" }}>
                        ...
                        <div style={{ display: "flex", alignItems: "center", gap: 16, flexWrap: "wrap" }}>
                            <div style={{ position: "relative", width: 120, height: 120, flexShrink: 0 }}>
                                <svg viewBox="0 0 36 36" style={{ width: "100%", height: "100%", transform: "rotate(-90deg)" }}>
                                    <circle cx="18" cy="18" r="15.9" fill="none" stroke="var(--cp-surface2)" strokeWidth="3" />
                                    <circle cx="18" cy="18" r="15.9" fill="none" stroke="var(--cp-success)" strokeWidth="3"
                                        strokeDasharray={`${data?.feeCollectionRate || 0} 100`} strokeLinecap="round" />
                                </svg>
                                <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column" }}>
                                    <span style={{ fontFamily: "'DM Serif Display',serif", fontSize: "1.4rem", color: "var(--cp-success)" }}>
                            ...
                            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                                <div>
                                    <div style={{ fontSize: 10, color: "var(--cp-muted)", fontWeight: 700, textTransform: "uppercase", letterSpacing: ".08em", marginBottom: 3 }}>Collected</div>
                                    <div style={{ fontFamily: "'DM Serif Display',serif", fontSize: "1.2rem", color: "var(--cp-success)" }}>{fmt(data?.totalRevenue || 0)}</div>
                                </div>
                                <div>
                                    <div style={{ fontSize: 10, color: "var(--cp-muted)", fontWeight: 700, textTransform: "uppercase", letterSpacing: ".08em", marginBottom: 3 }}>Pending</div>
                                    <div style={{ fontFamily: "'DM Serif Display',serif", fontSize: "1.2rem", color: "var(--cp-danger)" }}>{fmt(data?.totalPending || 0)}</div>
                                </div>
                            </div>
                        </div>
                    </div>
