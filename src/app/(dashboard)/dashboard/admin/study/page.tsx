...
// ── NoteIframeRenderer — SSR off (iframe use karta hai) ──
const NoteIframeRenderer = dynamic(
    () => import("@/components/lms/NoteIframeRanderer"),
    { ssr: false }
);
...
// ── Print helper ──────────────────────────────────────────
function printNote(title: string, moduleName: string, content: string, contentType: "markdown" | "html") {
    const win = window.open("", "_blank");
    if (!win) return;

    // HTML notes ke liye directly open karo
    if (contentType === "html") {
        const printHtml = content.includes("</body>")
            ? content.replace(
                "</body>",
                `<script>
    ...
    // Markdown notes ke liye existing print logic
...
<style>
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  <div class="header">
    <div class="academy">Shivshakti Computer Academy — Admin Preview</div>
    <div class="module-label">${moduleName}</div>
    <h1>${title}</h1>
  </div>
  <div id="content"></div>
  <div class="footer">
    <span>Admin Preview Copy</span>
...
export default function AdminStudyPage() {
    ...
    const [noteContentType, setNoteContentType] = useState<"markdown" | "html">("markdown"); // ← NEW
    const [globalCss,       setGlobalCss]       = useState("");  // ← NEW
    ...
    useEffect(() => {
        ...
        // Global CSS — HTML notes ke liye preview mein chahiye
        fetch("/api/public/notes-css")
            .then(r => r.json())
            .then(d => setGlobalCss(d.css || ""))
            .catch(console.error);

        // Mobile pe default sidebar closed
    ...
    const loadCourseNotes = useCallback(async (slug: string) => {
        ...
        try {
            const res = await fetch(
                `/api/admin/notes/study?courseSlug=${slug}`,
                { credentials: "include" }
            );
    ...
    const loadNote = async (note: NoteItem) => {
        ...
        setNoteContent("");
        setNoteTitle("");
        setNoteContentType("markdown");   // ← reset
        setNoteMeta(null);
        if (window.innerWidth < 769) setSidebarOpen(false);

        try {
            const res = await fetch(
                `/api/admin/notes/study/${note._id}`,
                { credentials: "include" }
            );
            const d = await res.json();
            if (res.ok) {
                ...
                setNoteContentType(d.note.contentType || "markdown");  // ← NEW
                setNoteMeta({
                    moduleName:  d.note.moduleName,
                    updatedAt:   d.note.updatedAt,
                    isPublished: d.note.isPublished,
                });
                setTimeout(() => articleRef.current?.scrollTo({
            ...
            <div className="tn-shell">
                ...
                <div className="tn-topbar">
                        ...
                        {selectedNote && noteMeta && (
                            <div className="tn-breadcrumb">
                                ...
                                <span style={{
                                    ...
                                    padding:      "2px 7px",
                                    borderRadius: 100,
                                    background:   noteContentType === "html"
                                        ? "rgba(16,185,129,.1)"
                                        : "rgba(100,116,139,.1)",
                                    color:        noteContentType === "html"
                                        ? "#059669"
                                        : "var(--cp-muted)",
                                    border:       `1px solid ${noteContentType === "html"
                                        ? "rgba(16,185,129,.2)"
                                        : "rgba(100,116,139,.2)"}`,
                                    marginLeft:   4,
                                    flexShrink:   0,
                                }}>
                                    {noteContentType === "html" ? "HTML" : "MD"}
                                </span>
                            </div>
                        )}
                    </div>
                    ...
                    <div className="tn-topbar-right">
                        <button
                            className={`tn-pill-btn${showDraft ? " tn-pill-btn--active" : ""}`}
                    ...
                    <aside className={`tn-sidebar${sidebarOpen ? " tn-sidebar--open" : ""}`}>
                        ...
                        <div className="tn-sb-head">
                            <div className="tn-sb-top">
                                ...
                                <button
                                    className="tn-icon-btn tn-sb-close"
                                        ...
                                        {isOpen && mod.notes.map(note => {
                                            ...
                                            return (
                                                <button
                                                    ...
                                                    className={[
                                                        "tn-note-btn",
                                                        isActive        ? "tn-note-btn--active" : "",
                                                        !note.isPublished ? "tn-note-btn--draft"  : "",
                                                    ].join(" ")}
                                                    onClick={() => loadNote(note)}
                                                >
                                                    <FileText
                                                        size={11}
                                                        style={{ color: "var(--cp-muted)", flexShrink: 0 }}
                                                    />
                                                    <span
                            ...
                            <div className="tn-fade-in">
                                <article className="tn-article">
                                    ...
                                    <div className="tn-article-head">
                                        <div style={{
                                            ...
                                            <div style={{
                                                ...
                                                {noteContentType === "html" ? "🌐 HTML Note" : "📝 Markdown"}
                                            </div>
                                        </div>
                                    ...
                                    {noteContentType === "html" ? (
                                        ...
                                        /* Markdown note — existing renderer */
                                    ...
                                    <div className="tn-article-footer">
                                        <span>Shivshakti Computer Academy — Admin Preview</span>
                                        <button
                                            className="tn-print-btn"
                                            onClick={() => noteMeta && printNote(
                                                noteTitle,
                                                noteMeta.moduleName,
                                                noteContent,
                                                noteContentType,
...
// ── Styles (same as before — koi change nahi) ─────────────
const tnStyles = `
    ...
    @media print {
        .tn-topbar,.tn-sidebar,.tn-article-footer,.tn-copy-btn,.tn-browse-btn { display:none !important; }
    }
`;