...
import {
    ...
    BookOpen, Award, User, Shield, BadgeCheck
} from "lucide-react";
...
export default function StudentDetail() {
    ...
    const fetchEnrollConfigs = async (courseId: string) => {
        if (!courseId) { setEnrollConfigs([]); return; }
        setLoadingCfg(true);
        try {
            const r = await fetchWithAuth(`/api/admin/course-franchise-configs?courseId=${courseId}`);
            const d = await r.json();
            setEnrollConfigs(Array.isArray(d) ? d : []);
        } catch { setEnrollConfigs([]); }
    ...
    const openAssignFranchise = async (e: any) => {
        ...
        try {
            ...
            const d = await r.json();
            setAssignConfigs(Array.isArray(d) ? d : []);
        } catch { setAssignConfigs([]); }
        finally { setAssignLoadingCfg(false); }
    };

    const saveAssignFranchise = async () => {
        if (!assigningEid) return;
    ...
    const enrollCourse = async () => {
        ...
        const d = await res.json();
        if (!res.ok) return alert(d.message);
        setSelectedCourse(""); setFeesTotal("");
        setEnrollConfigs([]); setEnrollFranchiseId("");
        loadStudent();
    };
    ...
    if (!student) return (
        <div style={{ color: "#ef4444", fontFamily: "sans-serif", padding: 20 }}>
            Student not found
        </div>
    );
            ...
            <div className="sd-root">
                ...
                <div className="sd-card">
                    ...
                    <div className="sd-row-wrap">
                        <select className="sd-select" value={courseStatus}
                            onChange={e => setCourseStatus(e.target.value)}>
                            <option value="active">Active</option>
                            <option value="completed">Completed</option>
                            <option value="dropped">Dropped</option>
                        </select>
                        <button className="sd-amber-btn" onClick={updateCourseStatus}>
                            Update Status
                        </button>
                    </div>
                </div>
                ...
                <div className="sd-card">
                    ...
                    <div style={{ padding: "16px 18px", display: "flex", flexDirection: "column", gap: 12 }}>
                        {/* Course select */}
                        <div className="sd-row-wrap" style={{ padding: 0 }}>
                            <select className="sd-select" value={selectedCourse}
                                onChange={e => {
                                    setSelectedCourse(e.target.value);
                                    setEnrollFranchiseId("");
                                    setFeesTotal("");
                                    fetchEnrollConfigs(e.target.value);
                                }}>
                        ...
                        {selectedCourse && (
                            loadingCfg ? (
                                <div style={{ fontSize: 12, color: "var(--cp-muted)" }}>Loading franchise options…</div>
                            ) : enrollConfigs.length > 0 ? (
                                <div>
                                    <div className="sd-section-label" style={{ padding: "0 0 6px", border: "none", marginBottom: 8 }}>
                                    ...
                                    <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                                        <label style={{
                                            ...
                                            border: `1px solid ${!enrollFranchiseId ? "var(--cp-accent)" : "var(--cp-border)"}`,
                                            background: !enrollFranchiseId ? "var(--cp-accent-glow)" : "var(--cp-surface2)",
                                            fontSize: 12, color: "var(--cp-text)",
                                        }}>
                                            <input type="radio" value="" checked={!enrollFranchiseId}
                                                onChange={() => { setEnrollFranchiseId(""); setFeesTotal(""); }}
                                        ...
                                        {enrollConfigs.map(cfg => {
                                            ...
                                            return (
                                                <label key={cfg._id} style={{
                                                    ...
                                                    padding: "7px 12px", borderRadius: 8, cursor: "pointer",
                                                    border: `1px solid ${sel ? color : "var(--cp-border)"}`,
                                                    background: sel ? "var(--cp-accent-glow)" : "var(--cp-surface2)",
                                                    fontSize: 12, color: "var(--cp-text)",
                                                }}>
                                                    <input type="radio" value={cfg.franchise._id}
                                                        checked={sel}
                                                        onChange={() => {
                                                            setEnrollFranchiseId(cfg.franchise._id);
                                                            setFeesTotal(cfg.feeStructure?.total
                                                    ...
                                                    <span>{cfg.franchise.name}</span>
                                                    {cfg.feeStructure?.total && (
                                                        <span style={{ color: "var(--cp-success)", fontSize: 11, fontWeight: 700 }}>
                                                            ₹{cfg.feeStructure.total.toLocaleString("en-IN")}
                                                        </span>
                                                    )}
                                                </label>
                                            );
                        ...
                        <div key={e._id} className="sd-card">
                            ...
                            <div className="sd-card-head">
                                <div className="sd-card-head-left">
                                    ...
                                    {/* Franchise badge in card head */}
                                    {e.franchise && (
                                        <span style={{
                                            fontSize: 9, fontWeight: 800,
                                            padding: "1px 7px", borderRadius: 4,
                                            background: e.franchise.isOwn ? "#F59E0B" : "var(--cp-accent)",
                                ...
                                <div style={{ display: "flex", gap: 8 }}>
                                    <button className="sd-link-btn" onClick={() => openAssignFranchise(e)}>
                                        <Shield size={10} /> {e.franchise ? "Change Franchise" : "Assign Franchise"}
                                    </button>
                                    <button className="sd-link-btn" onClick={() => {
                                        setEditingEnrollment(e);
                                        setEditFeesTotal(e.feesTotal?.toString() || "");
                                    }}>
                            ...
                            {/* Franchise info strip — only for new enrollments */}
                            {e.franchise && (
                                <div style={{
                                    margin: "0",
                                    padding: "10px 18px",
                                    ...
                                    <Shield size={12} style={{
                                        color: e.franchise.isOwn ? "#F59E0B" : "var(--cp-accent)",
                                        flexShrink: 0,
                                    }} />
                                    ...
                                    {e.certType && (
                                        <span style={{ fontSize: 11, color: "var(--cp-muted)" }}>
                                            · {e.certType.name} ({e.certType.issuingBody})
                                        </span>
                                    )}
                                    {/* Registered bodies */}
                                    {(e.franchise.registeredBodies?.length ?? 0) > 0 && (
                                        <div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
                                            {e.franchise.registeredBodies.map((b: string) => (
                                                <span key={b} style={{
                                    ...
                                    {/* External Student ID */}
                                    {e.externalStudentId && (
                                        <span style={{ fontSize: 11, color: "var(--cp-muted)", marginLeft: "auto" }}>
                                            {e.franchise.code} ID:{" "}
                                            <span style={{ fontFamily: "monospace", color: "var(--cp-text)" }}>
                                                {e.externalStudentId}
                                            </span>
                                        </span>
                                    )}
                ...
                {editingPayment && (
                    <div className="sd-overlay"
                        onClick={e => e.target === e.currentTarget && setEditingPayment(null)}>
                        <div className="sd-modal">
                            <div className="sd-modal-head">
                                <span className="sd-modal-title">Edit Payment</span>
                                <button className="sd-modal-close" onClick={() => setEditingPayment(null)}>
                                    <X size={13} />
                                </button>
                            </div>
                            <div className="sd-modal-body">
                                <div className="sd-field">
                                    ...
                                    <input className="sd-input" type="number" value={editAmount}
                                        onChange={e => setEditAmount(e.target.value)} />
                                </div>
                                <div className="sd-field">
                                    <label className="sd-label">Date</label>
                                    <input className="sd-input" type="date" value={editDate}
                                        onChange={e => setEditDate(e.target.value)} />
                                </div>
                                <div className="sd-field">
                                    <label className="sd-label">Remark</label>
                                    <input className="sd-input" placeholder="Optional remark" value={editRemark}
                ...
                {editingEnrollment && (
                    <div className="sd-overlay"
                        onClick={e => e.target === e.currentTarget && setEditingEnrollment(null)}>
                        <div className="sd-modal">
                            <div className="sd-modal-head">
                                <span className="sd-modal-title">Edit Enrollment Fees</span>
                                <button className="sd-modal-close" onClick={() => setEditingEnrollment(null)}>
                                    <X size={13} />
                                </button>
                            </div>
                            <div className="sd-modal-body">
                                <div className="sd-field">
                                    ...
                                    <input className="sd-input" type="number" value={editFeesTotal}
                ...
                {assigningEid && (
                    <div className="sd-overlay" onClick={e => e.target === e.currentTarget && setAssigningEid(null)}>
                        <div className="sd-modal" style={{ maxWidth: 520 }}>
                            <div className="sd-modal-head">
                                <span className="sd-modal-title">Assign Franchise</span>
                                <button className="sd-modal-close" onClick={() => setAssigningEid(null)}>
                                    <X size={13} />
                                </button>
                            </div>
                            <div className="sd-modal-body">
                                {assignLoadingCfg ? (
                                    <div style={{ fontSize: 13, color: "var(--cp-muted)", padding: "20px 0", textAlign: "center" }}>
                                    ...
                                    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                                        <div className="sd-label" style={{ marginBottom: 4 }}>Franchise select karo</div>

                                        {/* No franchise option */}
                                        <label style={{
                                            display: "flex", alignItems: "center", gap: 10,
                                            padding: "10px 14px", borderRadius: 9, cursor: "pointer",
                                            border: `1px solid ${!assignFranchiseId ? "var(--cp-accent)" : "var(--cp-border)"}`,
                                            background: !assignFranchiseId ? "var(--cp-accent-glow)" : "var(--cp-surface2)",
                                        }}>
                                            <input type="radio" name="assignFranchise" value=""
                                                checked={!assignFranchiseId}
                                            ...
                                            <div>
                                                ...
                                                <div style={{ fontSize: 10, color: "var(--cp-muted)" }}>
                                                    Franchise remove karo (legacy mode)
                                                </div>
                                            </div>
                                        </label>
                                        ...
                                        {assignConfigs.map((cfg: any) => {
                                            ...
                                            return (
                                                <label key={cfg._id} style={{
                                                    ...
                                                    <input type="radio" name="assignFranchise"
                                                        value={cfg.franchise._id}
                                                        checked={sel}
                                                        onChange={() => setAssignFranchiseId(cfg.franchise._id)}
                                                    ...
                                                    <div style={{ flex: 1 }}>
                                                        ...
                                                        <div style={{ fontSize: 11, color: "var(--cp-muted)" }}>
                                                            {cfg.defaultCertType?.name ?? cfg.certType?.name ?? "—"} · {cfg.defaultCertType?.issuingBody ?? cfg.certType?.issuingBody ?? "—"}
                                                        </div>
                                                        {cfg.feeStructure?.total && (
                                                            <div style={{ fontSize: 11, color: "var(--cp-success)", fontWeight: 600, marginTop: 3 }}>
                                                                Default Fee: ₹{cfg.feeStructure.total.toLocaleString("en-IN")}
                                                            </div>
                                                        )}
                                                    </div>
                                                </label>
                                ...
                                <div className="sd-modal-footer">
                                    ...
                                    <button className="sd-amber-btn" onClick={saveAssignFranchise}
                                        disabled={assignLoading || assignConfigs.length === 0}>
                                        {assignLoading ? "Saving…" : "Save Franchise"}
                                    </button>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="sd-overlay"
                        ...
                        <div className="sd-modal" style={{ maxWidth: 500 }}>
                            <div className="sd-modal-head">
                                ...
                                <button className="sd-modal-close" onClick={() => setEditProfileOpen(false)}>
                                    <X size={13} />
                                </button>
                            </div>
                            <div className="sd-modal-body">
                                {[
                                    ...
                                    <div key={f.name} className="sd-field">
                                        ...
                                        <input className="sd-input" type={f.type} name={f.name}
                                            value={(profileForm as any)[f.name]}
                                            onChange={handleProfileChange} />
                                    </div>
                                ))}
                                <div className="sd-field">
                                    <label className="sd-label">Gender</label>
                                    <select className="sd-select" name="gender" value={profileForm.gender}
                                        onChange={handleProfileChange}>
                                ...
                                <div className="sd-field">
                                    ...
                                    <textarea className="sd-textarea" name="address"
...
const sdStyles = `
    ...
    .sd-back-btn { display:inline-flex; align-items:center; gap:5px; padding:7px 13px; border-radius:8px; border:1px solid var(--cp-border); background:var(--cp-surface); color:var(--cp-subtext); font-size:12px; font-weight:500; cursor:pointer; font-family:'Plus Jakarta Sans',sans-serif; transition:all .14s; }
    ...
    .sd-card-head { display:flex; align-items:center; justify-content:space-between; padding:13px 18px; border-bottom:1px solid var(--cp-border); background:var(--cp-surface2); }
    .sd-card-head-left { display:flex; align-items:center; gap:7px; font-size:12px; font-weight:700; letter-spacing:.06em; text-transform:uppercase; color:var(--cp-subtext); }
    ...
    .sd-input, .sd-select, .sd-textarea { font-family:'Plus Jakarta Sans',sans-serif; padding:9px 12px; font-size:13px; background:var(--cp-bg); border:1px solid var(--cp-border); border-radius:8px; color:var(--cp-text); outline:none; transition:border-color .15s; width:100%; }
    ...
    .sd-section-label { font-size:10px; font-weight:700; letter-spacing:.1em; text-transform:uppercase; color:var(--cp-muted); padding:10px 18px 4px; border-top:1px solid var(--cp-border); }
    .sd-amber-btn { display:inline-flex; align-items:center; gap:6px; padding:9px 18px; border-radius:8px; border:none; cursor:pointer; font-family:'Plus Jakarta Sans',sans-serif; font-size:12px; font-weight:700; background:var(--cp-accent); color:#fff; transition:opacity .15s; white-space:nowrap; }
    .sd-amber-btn:hover { opacity:.88; }
    .sd-green-btn { display:inline-flex; align-items:center; gap:6px; padding:9px 18px; border-radius:8px; cursor:pointer; font-family:'Plus Jakarta Sans',sans-serif; font-size:12px; font-weight:700; background:rgba(34,197,94,0.15); color:var(--cp-success); border:1px solid rgba(34,197,94,0.3); transition:all .15s; white-space:nowrap; }
    .sd-green-btn:hover { background:rgba(34,197,94,0.25); }
    .sd-ghost-btn { padding:9px 18px; border-radius:8px; border:1px solid var(--cp-border); background:transparent; color:var(--cp-muted); font-size:12px; font-weight:600; cursor:pointer; font-family:'Plus Jakarta Sans',sans-serif; transition:all .14s; }
    .sd-ghost-btn:hover { border-color:var(--cp-border2); color:var(--cp-subtext); }
    .sd-link-btn { display:inline-flex; align-items:center; gap:4px; font-size:11px; font-weight:600; color:var(--cp-accent); background:none; border:none; cursor:pointer; font-family:'Plus Jakarta Sans',sans-serif; padding:4px 6px; border-radius:5px; transition:background .13s; }
    ...
    .sd-action-btn { display:inline-flex; align-items:center; gap:7px; padding:10px 20px; border-radius:9px; border:1px solid; cursor:pointer; font-family:'Plus Jakarta Sans',sans-serif; font-size:12px; font-weight:700; transition:all .15s; }
    ...
    .sd-modal { background:var(--cp-surface); border:1px solid var(--cp-border); border-radius:14px; width:100%; max-width:440px; max-height:90vh; overflow-y:auto; box-shadow:0 24px 60px rgba(0,0,0,.5); scrollbar-width:thin; scrollbar-color:var(--cp-border2) transparent; animation:sdModalIn .18s ease; }
    @keyframes sdModalIn { from{opacity:0;transform:scale(.95)} to{opacity:1;transform:scale(1)} }
    .sd-modal-head { display:flex; align-items:center; justify-content:space-between; padding:16px 20px; border-bottom:1px solid var(--cp-border); position:sticky; top:0; background:var(--cp-surface); z-index:2; }
    .sd-modal-title { font-family:'DM Serif Display',serif; font-size:1.05rem; color:var(--cp-text); }
    .sd-modal-close { width:28px; height:28px; border-radius:7px; border:1px solid var(--cp-border); background:transparent; cursor:pointer; display:flex; align-items:center; justify-content:center; color:var(--cp-muted); transition:all .14s; }
    ...
    .sd-skeleton { background:linear-gradient(90deg, var(--cp-surface) 25%, var(--cp-surface2) 50%, var(--cp-surface) 75%); background-size:200% 100%; animation:sdShimmer 1.4s infinite; }
