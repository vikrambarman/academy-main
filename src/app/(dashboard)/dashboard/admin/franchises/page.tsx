...
import { Plus, X, Edit2, ToggleLeft, ToggleRight, Shield, Award, ChevronDown, ChevronUp } from "lucide-react";
...
const EMPTY_F = { name: "", code: "", description: "", registeredBodies: [""], websiteUrl: "", portalUrl: "", portalLoginRequired: false, isOwn: false };
const EMPTY_C = { franchiseId: "", name: "", code: "", issuingBody: "", verificationMethod: "", verificationUrl: "", benefits: [""], defaultFee: "", description: "" };
...
export default function AdminFranchisesPage() {
    ...
    const saveF = async () => {
        if (!formF.name || !formF.code) { showToast("Name aur Code required", false); return; }
        setSaving(true);
        try {
            const method = editF ? "PUT" : "POST";
            const body = {
                ...(editF && { id: editF._id }),
                ...formF,
    ...
    const openEditC = (c: CertType) => {
        ...
        setFormC({
            ...
            verificationUrl: c.verificationUrl || "", benefits: c.benefits.length ? c.benefits : [""],
    ...
    const saveC = async () => {
        ...
        try {
            ...
            const d = await res.json();
            if (!res.ok) { showToast(d.message || "Error", false); return; }
            showToast(editC ? "Updated" : "Created");
            setCModal(false);
            load();
        } finally { setSaving(false); }
    };
    ...
    return (
        <>
            <style>{styles}</style>
            {toast && <div className={`af-toast ${toast.ok ? "ok" : "err"}`}>{toast.msg}</div>}
            <div className="af-root">
                ...
                {franchises.length === 0 ? (
                    <div className="af-empty">
                        <Shield size={28} style={{ opacity: .3, marginBottom: 10 }} />
                        <div>No franchises yet. Add GSDM, Drishti ya apna khud ka.</div>
                        <div style={{ fontSize: 11, marginTop: 6 }}>Pehle seed script chalao: <code>npx ts-node scripts/seedFranchises.ts</code></div>
                    </div>
                ) : franchises.map(f => {
                    const fCerts = certTypes.filter(c => c.franchise._id === f._id || c.franchise._id?.toString() === f._id?.toString());
                    const isOpen = expanded.includes(f._id);
                    return (
                        <div key={f._id} className={`af-card ${!f.isActive ? "inactive" : ""}`}>
                            <div className="af-card-head">
                                <div className="af-card-left">
                                    <div className="af-code-badge" style={{ background: f.isOwn ? "rgba(245,158,11,0.15)" : "var(--cp-accent-glow)", color: f.isOwn ? "var(--cp-warning)" : "var(--cp-accent)" }}>
                                        {f.code}
                                    </div>
                                    <div>
                                ...
                                <div className="af-card-right">
                                    <span className={`af-status ${f.isActive ? "on" : "off"}`}>
                                        {f.isActive ? "Active" : "Inactive"}
                                    </span>
                                    <button className="af-icon-btn" onClick={() => openEditF(f)} title="Edit"><Edit2 size={12} /></button>
                                    <button className="af-icon-btn toggle" onClick={() => toggleF(f)} title="Toggle">
                                        {f.isActive ? <ToggleRight size={16} style={{ color: "var(--cp-success)" }} /> : <ToggleLeft size={16} />}
                                    </button>
                                    <button className="af-icon-btn expand" onClick={() => toggleExpand(f._id)}>
                            ...
                            {isOpen && (
                                <div className="af-cert-section">
                                    ...
                                    {fCerts.length === 0 ? (
                                        <div className="af-cert-empty">No cert types for {f.name} — add one.</div>
                                    ) : (
                                        <div className="af-cert-grid">
                                            {fCerts.map(c => (
                                                <div key={c._id} className="af-cert-card">
                                                    <div className="af-cert-code">{c.code}</div>
                                                    <div className="af-cert-name">{c.name}</div>
                                                    <div className="af-cert-body">{c.issuingBody}</div>
                                                    {c.verificationMethod && (
                                                        <div className="af-cert-verify">{c.verificationMethod}</div>
                                                    )}
                                                    <div className="af-cert-fee">Default fee: ₹{c.defaultFee.toLocaleString("en-IN")}</div>
                                                    <button className="af-link-btn" onClick={() => openEditC(c)}>
                                                        <Edit2 size={10} /> Edit
                                                    </button>
                                                </div>
                                            ))}
                        ...
                        <div className="af-modal-body">
                            <div className="af-form-grid">
                                <Field label="Franchise Name *">
                                    <input className="af-input" value={formF.name} onChange={e => setFormF(p => ({ ...p, name: e.target.value }))} placeholder="e.g. Gramin Skill Development Mission" />
                                </Field>
                                <Field label="Code * (uppercase, no spaces)">
                                    <input className="af-input" value={formF.code} onChange={e => setFormF(p => ({ ...p, code: e.target.value.toUpperCase().replace(/\s/g, "") }))} placeholder="e.g. GSDM" disabled={!!editF} />
                                </Field>
                                <Field label="Website URL">
                                    <input className="af-input" value={formF.websiteUrl} onChange={e => setFormF(p => ({ ...p, websiteUrl: e.target.value }))} placeholder="https://..." />
                                </Field>
                                <Field label="Portal/Login URL">
                                    <input className="af-input" value={formF.portalUrl} onChange={e => setFormF(p => ({ ...p, portalUrl: e.target.value }))} placeholder="https://..." />
                                </Field>
                            </div>
                            <Field label="Description">
                                <textarea className="af-input" style={{ minHeight: 70, resize: "vertical" }} value={formF.description} onChange={e => setFormF(p => ({ ...p, description: e.target.value }))} />
                            </Field>
                            <Field label="Registered Bodies (MSME, MCA, NSDC, etc.)">
                                {formF.registeredBodies.map((b, i) => (
                                    <div key={i} style={{ display: "flex", gap: 6, marginBottom: 6 }}>
                                        <input className="af-input" style={{ flex: 1 }} value={b} onChange={e => { const u = [...formF.registeredBodies]; u[i] = e.target.value; setFormF(p => ({ ...p, registeredBodies: u })); }} placeholder="e.g. NSDC" />
                                        {formF.registeredBodies.length > 1 && (
                                            <button className="af-icon-btn danger" onClick={() => setFormF(p => ({ ...p, registeredBodies: p.registeredBodies.filter((_, j) => j !== i) }))}>
                                                <X size={11} />
                                            </button>
                                        )}
                                    </div>
                                ))}
                                <button className="af-link-btn" onClick={() => setFormF(p => ({ ...p, registeredBodies: [...p.registeredBodies, ""] }))}>
                                    <Plus size={10} /> Add body
                                </button>
                            </Field>
                            <div className="af-checkbox-row">
                                <label className="af-checkbox">
                                    <input type="checkbox" checked={formF.portalLoginRequired} onChange={e => setFormF(p => ({ ...p, portalLoginRequired: e.target.checked }))} />
                                    Portal login required
                                </label>
                                <label className="af-checkbox">
                                    <input type="checkbox" checked={formF.isOwn} onChange={e => setFormF(p => ({ ...p, isOwn: e.target.checked }))} />
                        ...
                        <div className="af-modal-body">
                            {!editC && (
                                <Field label="Franchise *">
                                    <select className="af-input" value={formC.franchiseId} onChange={e => setFormC(p => ({ ...p, franchiseId: e.target.value }))}>
                            ...
                            <div className="af-form-grid">
                                <Field label="Cert Type Name *">
                                    <input className="af-input" value={formC.name} onChange={e => setFormC(p => ({ ...p, name: e.target.value }))} placeholder="e.g. NSDC Short Term Certificate" />
                                </Field>
                                {!editC && (
                                    <Field label="Code * (uppercase, no spaces)">
                                        <input className="af-input" value={formC.code} onChange={e => setFormC(p => ({ ...p, code: e.target.value.toUpperCase().replace(/\s/g, "") }))} placeholder="e.g. GSDM_NSDC" />
                                    </Field>
                                )}
                                <Field label="Issuing Body *">
                                    <input className="af-input" value={formC.issuingBody} onChange={e => setFormC(p => ({ ...p, issuingBody: e.target.value }))} placeholder="e.g. NSDC / Skill India" />
                                </Field>
                                <Field label="Verification Method">
                                    <input className="af-input" value={formC.verificationMethod} onChange={e => setFormC(p => ({ ...p, verificationMethod: e.target.value }))} placeholder="e.g. DigiLocker + NSDC Portal" />
                                </Field>
                                <Field label="Verification URL">
                                    <input className="af-input" value={formC.verificationUrl} onChange={e => setFormC(p => ({ ...p, verificationUrl: e.target.value }))} placeholder="https://digilocker.gov.in" />
                                </Field>
                                <Field label="Default Fee (₹)">
                                    <input className="af-input" type="number" value={formC.defaultFee} onChange={e => setFormC(p => ({ ...p, defaultFee: e.target.value }))} placeholder="e.g. 2500" />
                                </Field>
                            </div>
                            <Field label="Benefits (student ko ye dikhega)">
                                {formC.benefits.map((b, i) => (
                                    <div key={i} style={{ display: "flex", gap: 6, marginBottom: 6 }}>
                                        <input className="af-input" style={{ flex: 1 }} value={b} onChange={e => { const u = [...formC.benefits]; u[i] = e.target.value; setFormC(p => ({ ...p, benefits: u })); }} placeholder="e.g. DigiLocker mein stored" />
                                        {formC.benefits.length > 1 && (
                                            <button className="af-icon-btn danger" onClick={() => setFormC(p => ({ ...p, benefits: p.benefits.filter((_, j) => j !== i) }))}>
                                                <X size={11} />
                                            </button>
                                        )}
                                    </div>
                                ))}
                                <button className="af-link-btn" onClick={() => setFormC(p => ({ ...p, benefits: [...p.benefits, ""] }))}>
...
const styles = `
    ...
    .af-toast { position:fixed; top:16px; right:16px; z-index:999; padding:10px 18px; border-radius:9px; font-size:12px; font-weight:700; font-family:'Plus Jakarta Sans',sans-serif; }
    ...
    .af-icon-btn:hover { background:var(--cp-accent-glow); color:var(--cp-accent); border-color:var(--cp-accent); }
    .af-icon-btn.danger { background:rgba(239,68,68,0.08); color:var(--cp-danger); border-color:rgba(239,68,68,0.2); }
    .af-icon-btn.toggle { width:auto; padding:0 6px; background:transparent; border:none; }
    ...
    .af-btn-sm { display:inline-flex; align-items:center; gap:4px; padding:5px 12px; border-radius:7px; border:1px solid var(--cp-border); background:var(--cp-surface); color:var(--cp-subtext); font-size:11px; font-weight:600; cursor:pointer; font-family:'Plus Jakarta Sans',sans-serif; }
    .af-btn-sm:hover { border-color:var(--cp-accent); color:var(--cp-accent); }
    .af-btn-ghost { padding:9px 16px; border-radius:8px; border:1px solid var(--cp-border); background:transparent; color:var(--cp-muted); font-size:12px; font-weight:600; cursor:pointer; font-family:'Plus Jakarta Sans',sans-serif; }
    .af-link-btn { display:inline-flex; align-items:center; gap:4px; background:none; border:none; cursor:pointer; font-size:11px; font-weight:600; color:var(--cp-accent); font-family:'Plus Jakarta Sans',sans-serif; padding:3px 0; }
    ...
    .af-input, .af-input select { font-family:'Plus Jakarta Sans',sans-serif; width:100%; padding:9px 12px; font-size:13px; background:var(--cp-bg); border:1px solid var(--cp-border); border-radius:8px; color:var(--cp-text); outline:none; transition:border-color .15s; }
