...
import {
    Plus, Trash2, Save, ChevronDown, Clock,
    BookOpen, User, MapPin, AlertCircle, CheckCircle,
...
// Per-day accent colors — dark-mode safe via CSS variables fallback
const DAY_PILL: Record<WeekDay, string> = {
    ...
    return (
        <div className="at-root">
            <div className="at-header">
                <div>
                    <div className="at-sk" style={{ width: 200, height: 28, borderRadius: 6 }} />
                    <div className="at-sk" style={{ width: 260, height: 12, borderRadius: 4, marginTop: 8 }} />
                </div>
            </div>
            <div className="at-sk" style={{ width: 280, height: 42, borderRadius: 12 }} />
            <div className="at-sk" style={{ width: "100%", height: 120, borderRadius: 14 }} />
        </div>
    );
}
...
export default function AdminTimetablePage() {
    ...
    useEffect(() => {
        ...
        fetchWithAuth("/api/admin/courses")
            ...
            .catch(() => showToast("error", "Courses load nahi hue"))
    ...
    // Load existing timetable (single mode)
    useEffect(() => {
    ...
    const switchMode = (m: Mode) => {
        setMode(m); setEnrollmentId("");
        setSelectedStudents(new Set()); setExisting(null); setSlots([EMPTY_SLOT()]);
    };
    ...
    // Bulk helpers
    const toggleStudent = (eid: string) => setSelectedStudents(prev => {
        const next = new Set(prev); next.has(eid) ? next.delete(eid) : next.add(eid); return next;
    });
    const toggleAll = () => setSelectedStudents(
        selectedStudents.size === enrollments.length ? new Set() : new Set(enrollments.map(e => e._id))
    );

    const handleSave = async () => {
        if (!courseId) return showToast("error", "Course select karo");
        if (slots.some(s => !s.subject || !s.startTime || !s.endTime))
            return showToast("error", "Saare slots complete karo");
        ...
        try {
            if (mode === "bulk") {
                ...
                const res = await fetchWithAuth("/api/admin/timetable", {
                    ...
                    body: JSON.stringify({
                        courseId, bulk: true, students, slots,
                        validFrom, validTo: validTo || undefined,
                ...
                if (!enroll) return showToast("error", "Student select karo");
                ...
                const res = await fetchWithAuth("/api/admin/timetable", {
                    ...
                    body: JSON.stringify(
                        existing
                            ? { timetableId: existing._id, slots, validTo: validTo || undefined }
                            : {
    ...
    const showBuilder    = mode === "bulk" ? (courseId && selectedStudents.size > 0) : !!enrollmentId;
    const courseName     = courses.find(c => c._id === courseId)?.name;

    if (pageLoading) return <><style>{css}</style><TimetableSkeleton /></>;
            ...
            <div className="at-root">
                ...
                <div className="at-header">
                    ...
                    <div style={{ display: "flex", gap: 8 }}>
                        <button className="at-icon-btn" title="Refresh courses"
                            onClick={() => fetchWithAuth("/api/admin/courses").then(r => r.json()).then(d => setCourses(Array.isArray(d) ? d : (d.courses || [])))}>
                            <RefreshCw size={14} />
                        </button>
                        {showBuilder && (
                            <button className="at-save-btn" onClick={handleSave} disabled={saving}>
                                <Save size={14} />
                                {saving ? "Saving..." : existing ? "Update Timetable" : "Create Timetable"}
                            </button>
                        )}
                    </div>
                </div>
                ...
                <div className="at-mode-toggle">
                    <button className={`at-mode-btn${mode === "single" ? " at-mode-btn--active" : ""}`}
                        onClick={() => switchMode("single")}>
                        <UserCheck size={14} /> Single Student
                    </button>
                    <button className={`at-mode-btn${mode === "bulk" ? " at-mode-btn--active" : ""}`}
                ...
                <div className="at-selectors">
                    ...
                    {mode === "single" && courseId && (
                        <div className="at-field">
                            ...
                            <div className="at-select-wrap">
                                <select className="at-select" value={enrollmentId}
                    ...
                    {showBuilder && (
                        <div className="at-dates">
                            <div className="at-field">
                                ...
                                <input type="date" className="at-input" value={validFrom}
                                    onChange={e => setValidFrom(e.target.value)} />
                            </div>
                            <div className="at-field">
                                <label className="at-label">Valid To (optional)</label>
                                <input type="date" className="at-input" value={validTo}
                ...
                {mode === "bulk" && courseId && (
                    <div className="at-bulk-panel">
                        <div className="at-bulk-head">
                            <span className="at-bulk-title">
                                <Users size={13} />
                                {enrollLoading ? "Loading..." : `${enrollments.length} students enrolled`}
                                {courseName && <span className="at-bulk-course"> · {courseName}</span>}
                            </span>
                            <button className="at-select-all-btn" onClick={toggleAll}>
                                {selectedStudents.size === enrollments.length && enrollments.length > 0
                                    ? "Deselect All" : "Select All"}
                            </button>
                        </div>

                        {enrollLoading ? (
                            <div className="at-bulk-loading">
                                <div className="at-spinner" /> Loading students...
                            </div>
                            ...
                            <div className="at-bulk-list">
                                {enrollments.map(e => {
                                    ...
                                    return (
                                        <label key={e._id} className={`at-bulk-row${checked ? " at-bulk-row--checked" : ""}`}>
                                            <input type="checkbox" checked={checked}
                                                onChange={() => toggleStudent(e._id)}
                                                style={{ accentColor: "var(--cp-accent)" }} />
                                            <div className="at-bulk-avatar">
                                            ...
                                            {checked && (
                                                <CheckCircle size={14} style={{ marginLeft: "auto", flexShrink: 0, color: "var(--cp-accent)" }} />
                                            )}
                                        </label>
                                    );
                                })}
                ...
                {showBuilder && (
                    <>
                        <div className="at-slots-heading">
                            <span className="at-slots-label">Class Slots</span>
                            <div className="at-day-pills">
                                <button
                                    className={`at-day-pill${activeDay === "all" ? " at-day-pill--all" : ""}`}
                                    onClick={() => setActiveDay("all")}>
                                    All ({slots.length})
                                </button>
                                {slotsByDay.map(d => {
                                    const isActive = activeDay === d.key;
                                    const color    = DAY_PILL[d.key];
                                    return (
                                        <button key={d.key}
                                            className={`at-day-pill${isActive ? " at-day-pill--day" : ""}`}
                                            style={isActive ? { borderColor: color, color } : {}}
                                            onClick={() => setActiveDay(isActive ? "all" : d.key)}>
                                            {d.short}
                                            {d.count > 0 && (
                                                <span className="at-day-count" style={{ background: color }}>
                                                    {d.count}
                                                </span>
                                            )}
                                        </button>
                        ...
                        <div className="at-slots">
                            {filteredSlots.length === 0 ? (
                                ...
                                const color = DAY_PILL[slot.day];
                                return (
                                    <div key={slot._idx} className="at-slot-card"
...
.at-root { font-family:'Plus Jakarta Sans',sans-serif; color:var(--cp-text); display:flex; flex-direction:column; gap:18px; max-width:900px; margin:0 auto; padding-bottom:60px; }

.at-toast { position:fixed; top:20px; right:24px; z-index:9999; display:flex; align-items:center; gap:8px; font-size:13px; font-weight:600; padding:12px 18px; border-radius:12px; box-shadow:0 8px 32px rgba(0,0,0,.35); animation:atIn .25s ease; }
...
@keyframes atIn { from{opacity:0;transform:translateY(-10px)} to{opacity:1;transform:translateY(0)} }
...
.at-icon-btn { width:36px; height:36px; border-radius:9px; border:1px solid var(--cp-border); background:var(--cp-surface); color:var(--cp-muted); cursor:pointer; display:flex; align-items:center; justify-content:center; transition:all .14s; }
.at-icon-btn:hover { border-color:var(--cp-accent); color:var(--cp-accent); background:var(--cp-accent-glow); }
...
.at-mode-btn--active { background:var(--cp-surface); color:var(--cp-accent); box-shadow:0 1px 4px rgba(0,0,0,.12); }
.at-mode-btn:hover:not(.at-mode-btn--active) { color:var(--cp-subtext); }
...
.at-select option { background:var(--cp-surface); }
...
.at-input::placeholder { color:var(--cp-muted); }

.at-bulk-panel { background:var(--cp-surface); border:1px solid var(--cp-border); border-radius:14px; overflow:hidden; }
.at-bulk-head  { display:flex; align-items:center; justify-content:space-between; padding:12px 16px; border-bottom:1px solid var(--cp-border); background:var(--cp-surface2); }
.at-bulk-title { display:flex; align-items:center; gap:7px; font-size:12px; font-weight:700; color:var(--cp-subtext); }
.at-bulk-course { color:var(--cp-muted); font-weight:500; }
.at-bulk-loading { display:flex; align-items:center; gap:10px; padding:20px; font-size:12px; color:var(--cp-muted); }
...
.at-bulk-row--checked { background:color-mix(in srgb,var(--cp-accent) 5%,var(--cp-surface)); }
...
.at-slots-label   { font-size:10px; font-weight:700; text-transform:uppercase; letter-spacing:.1em; color:var(--cp-muted); }
...
.at-day-pill--all { background:var(--cp-surface2); color:var(--cp-text); border-color:var(--cp-border2); }
...
.at-slot-row  { display:flex; gap:10px; flex-wrap:wrap; align-items:flex-end; }
...
.at-del-btn:hover { border-color:rgba(239,68,68,0.4); color:var(--cp-danger); background:rgba(239,68,68,0.08); }
...
.at-bottom-bar { display:flex; align-items:center; justify-content:space-between; padding:14px 18px; background:var(--cp-surface); border:1px solid var(--cp-border); border-radius:12px; position:sticky; bottom:0; box-shadow:0 -4px 16px rgba(0,0,0,.1); }
...
.at-empty { background:var(--cp-surface); border:1px dashed var(--cp-border); border-radius:12px; padding:48px; text-align:center; color:var(--cp-muted); font-size:13px; display:flex; flex-direction:column; align-items:center; }

.at-spinner { width:18px; height:18px; border:2px solid var(--cp-border); border-top-color:var(--cp-accent); border-radius:50%; animation:atSpin .7s linear infinite; }
@keyframes atSpin { to{transform:rotate(360deg)} }

.at-sk { background:linear-gradient(90deg,var(--cp-surface) 25%,var(--cp-surface2) 50%,var(--cp-surface) 75%); background-size:200% 100%; animation:atShimmer 1.4s infinite; }
@keyframes atShimmer { 0%{background-position:200% 0} 100%{background-position:-200% 0} }

@media(max-width:600px){
    .at-slot-row { flex-direction:column; }
